...
# Configuration
experiment_type = "human-ai"  # Change to "AI-only" or "Human-AI" as needed
...
aligned_team_data_dir = r"C:\Users\groessli\Documents\GitHub\interactive-gym-chase\data_preprocessing\AI_speed\aggregated_data\AI_speed_5\aligned_team_data"
...
if not is_human_ai:
    # ============================================================================
    # HUMAN-ONLY / AI-ONLY CONDITION: TEAM-BASED BONUS CALCULATION
    # ============================================================================
    ...
    # Report team deliveries sanity check
    ...
    # ============================================================================
    # HUMAN-AI CONDITION: INDIVIDUAL BONUS CALCULATION
    # ============================================================================
    
    bonus_data = []
    
    # Process all categories from individual subject files
    for subject_id, category_num, file_path in subject_data:
        df = pd.read_csv(file_path)
        
        # Categories 1, 2, 5: No compensation
        if category_num in [1, 2, 5]:
            bonus = 0.00
        # Categories 3, 4: Only compensate if completed all episodes
        elif category_num in [3, 4]:
            # Check if subject completed all episodes
            if 'episode_num' in df.columns:
                subject_max_episode = df['episode_num'].max()
                if subject_max_episode >= bonus_threshold:
                    # Subject completed all episodes: calculate bonus
                    if 'infos.0.delivery_reward' in df.columns:
                        total_delivery_reward = df['infos.0.delivery_reward'].sum()
                    else:
                        # Fallback to rewards.0
                        total_delivery_reward = df['rewards.0'].sum()
                    bonus = total_delivery_reward * 0.02
                else:
                    # Subject did not complete all episodes: no bonus
                    bonus = 0.00
            else:
                bonus = 0.00
        else:
...
if is_human_ai:
    # ============================================================================
    # HUMAN-AI CONDITION: VISUALIZATIONS
    # ============================================================================
    
    # Generate mean rewards plot (only for categories 3 & 4 with episode data)
    output_path = Path(fig_dir) / "mean_rewards_by_episode.png"
    ...
    if not output_path.exists():
        ...
        # Only process categories 3 and 4 (those with episode data)
        for subject_id, category_num, file_path in subject_data:
            if category_num not in [3, 4]:
                continue
            ...
            # Calculate sum of rewards for each episode
            if 'infos.0.delivery_act_reward' in df.columns:
                delivery_act_reward = df.groupby("episode_num")["infos.0.delivery_act_reward"].sum()
                onion_in_pot_reward = df.groupby("episode_num")["infos.0.onion_in_pot_reward"].sum()
            else:
                # Fallback
        ...
        if all_delivery_act_rewards and all_onion_in_pot_rewards:
            ...
            plt.xlabel('Episode', fontsize=12)
            plt.ylabel('Mean Reward', fontsize=12)
            plt.title('Human-AI Condition: Mean Rewards by Episode (Categories 3 & 4)', fontsize=14, fontweight='bold')
            plt.legend(fontsize=11)
            plt.grid(True, alpha=0.3)
            plt.xticks(episode_nums)
    ...
    # Generate histogram of compensation distribution (only completed subjects)
    histogram_path = Path(fig_dir) / "compensation_distribution_histogram.png"
    
    if not histogram_path.exists():
        # Filter to only completed subjects (those with compensation > 0)
        completed_df = bonus_df[bonus_df['compensation'] > 0]
        
        if len(completed_df) > 0:
            plt.figure(figsize=(10, 6))
            plt.hist(completed_df['compensation'], bins=15, color='steelblue', edgecolor='black', alpha=0.7)
            plt.xlabel('Compensation Amount ($)', fontsize=12)
            plt.ylabel('Number of Subjects', fontsize=12)
            plt.title('Human-AI Condition: Distribution of Compensation\n(Only subjects who completed all episodes)', fontsize=14, fontweight='bold')
    ...
    # ============================================================================
    # HUMAN-ONLY / AI-ONLY CONDITION: VISUALIZATIONS
    # ============================================================================
    
    # Mean rewards plot (from commented section, now activated)
    output_path = Path(fig_dir) / "mean_rewards_by_episode.png"
    ...
    if not histogram_path.exists():
        ...
        plt.xlabel('Compensation Amount ($)', fontsize=12)
        plt.ylabel('Number of Subjects', fontsize=12)
        plt.title('Distribution of Compensation by Category\n(Cat1=$0.50, Cat2=$0, Cat3&4=performance-based)', fontsize=14, fontweight='bold')
        plt.grid(True, alpha=0.3, axis='y')

        # Add vertical line at 0.50 to highlight category 1 subjects
        cat1_count = (bonus_df['compensation'] == 0.50).sum()
        if cat1_count > 0:
            plt.axvline(x=0.50, color='red', linestyle='--', linewidth=2, alpha=0.7, label=f'Category 1 ($0.50, {cat1_count} subjects)')
            plt.legend(fontsize=11)
